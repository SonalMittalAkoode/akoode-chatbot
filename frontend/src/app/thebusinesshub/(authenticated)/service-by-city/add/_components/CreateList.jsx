...
import { addServiceByCityAPI, generateSBCCitySeoSuggestionsAPI } from "@/api/serviceByCity";
import { getPublicSBCList } from "@/api/serviceByCountry";
import { getBlogTableData } from "@/api/blog";
import { importServiceByCityDocxAPI } from "@/api/docxImport";
import DocxImportPanel from "@/components/admin/DocxImportPanel";
import { getIndustryTableData } from "@/api/industry";
import { applyIndustryAutofill } from "@/utils/industryAutofill";
import { analyzeForTemplate, industriesAreFreeForm, mergeFixedStageSteps, mergeFixedTaxonomyIndustries } from "@/utils/docxTemplateCompat";
...
import BlogPicker from "@/components/admin/BlogPicker";
...
const buildStaticIndustries = () =>
  STATIC_INDUSTRIES.map(name => ({ id: uid(), name, points: [""] }));
...
const emptyIndustry = () => ({ id: uid(), icon: "", iconImg: null, iconImgPreview: "", name: "", points: [""] });
const emptyFaq = () => ({ id: uid(), question: "", answer: "" });
const emptyPlatformRating = () => ({ id: uid(), name: "", img: null, imgPreview: "", imgAlt: "", rating: "" });
const emptyClientLove = () => ({ id: uid(), icon: "", title: "", body: "" });
const emptyProcessService = () => ({ id: uid(), n: "", title: "", subtitle: "", para: "", points: [""], tags: [""] });
const emptyWhatWeDoStep = () => ({ id: uid(), shortTitle: "", title: "", body: "", timeline: "", timelineNote: "", deliverables: [""] });
const emptyTechCat = () => ({ id: uid(), title: "", icon: "", iconImg: null, iconImgPreview: "", desc: "", pills: [{ e: "", label: "", img: null, imgPreview: "" }] });
...
const SECTION_NAV = [
  ...
  { id: "sec-process",     label: "Services Offered" },
...
const CreateList = () => {
  ...
  // ── Country options from SBC ───────────────────────────────────────────────
  // ── SBC options for country picker ────────────────────────────────────────
  const [sbcOptions, setSbcOptions] = useState([]); // [{ _id, title, country, market, template }]
  useEffect(() => {
    getPublicSBCList().then(data => {
      const items = data?.items || [];
      setSbcOptions(items.map(item => ({
        _id: item._id,
        title: item.title || item.country || item.market || "",
  ...
  const [countryRef, setCountryRef] = useState(""); // ObjectId of linked SBC
  const [country, setCountry] = useState(""); // denormalized, read-only display
  const [market, setMarket] = useState(""); // denormalized, read-only display
  const [city, setCity] = useState("");
  const [citySlug, setCitySlug] = useState("");
  const [citySlugEdited, setCitySlugEdited] = useState(false);
  ...
  const filteredSbcOptions = sbcOptions.filter(
    (opt) => opt.template === template
  );
  ...
  const handleCountrySelect = (e) => {
    ...
    const opt = sbcOptions.find(o => o._id === selectedId);
    setCountryRef(selectedId);
    setCountry(opt ? opt.country : "");
    setMarket(opt ? opt.market : "");
  };
  const handleCityChange = (e) => {
    const val = e.target.value;
    setCity(val);
    if (!citySlugEdited) setCitySlug(toSlug(val));
  };
  const handleCitySlugChange = (e) => {
    setCitySlug(e.target.value);
    setCitySlugEdited(true);
  };
  ...
  const updatePlatformRatingImg = (id, file) => {
    setPlatformRatings(p => p.map(x => x.id === id ? {
      ...x,
      img: file || null,
      imgPreview: file ? URL.createObjectURL(file) : "",
      imgAlt: file ? (x.imgAlt || fileNameToAlt(file.name)) : "",
    } : x));
  };
  ...
  const uploadWhyImage = (e) => {
    const file = e.target.files[0];
    setWhyImage(file || null);
    setWhyImagePreview(file ? URL.createObjectURL(file) : "");
  ...
  const [processCtaText, setProcessCtaText] = useState("");
  const [processCtaLink, setProcessCtaLink] = useState("");
  ...
  const [whatWeDoSteps, setWhatWeDoSteps] = useState(buildStaticSteps());
  ...
  const addTechCatPill = (catId) => {
    setTechCats(p => p.map(x => x.id === catId ? { ...x, pills: [...x.pills, { e: "", label: "", img: null, imgPreview: "" }] } : x));
  };
  ...
  const updateIndustryIconImg = (id, file) => {
    setIndustries(p => p.map(x => x.id !== id ? x : {
  ...
  // ── Toggles + Blog/Case Studies pickers ───────────────────────────────────
  ...
  const [blogSelected, setBlogSelected] = useState(["", "", ""]); // "" = auto (latest)
  ...
  // ── AI SEO Suggestion helpers ──────────────────────────────────────────────
  // Build setters for the shared applier. Critically, slug, citySlug, market,
  // and city are URL-structure values — the AI must NOT overwrite them. We
  // pass NO-OP setters for those so the form's existing values are preserved.
  const buildSuggestionSetters = () => ({
    ...
    setCaseStudiesShow,
    setCaseStudiesHeading,
    setCaseStudiesSubtitle,
    setTestimonialsShow,
  ...
  const getCoreInfoValues = () => ({
    title, slug, country, market, city, citySlug, countryRef,
    keywords: "",
  });

  // ── DOCX import ────────────────────────────────────────────────────────────
  // Reuses the exact same applier the AI-suggestion flow already uses
  // (applySbcSeoSuggestion) for every rich-content section, so the DOCX
  // importer never duplicates form-population logic. Unlike the AI flow, the
  // importer is allowed to write slug/city/country/market — that's the whole
  // point of importing core info — so real setters are passed here instead
  // of buildSuggestionSetters()'s no-ops for those fields.
  const hasExistingCityFormData = Boolean(
    title.trim() || heroHeading.trim() || city.trim() || processServices.length || faqs.length
  );
  const docxTemplateWarnings = (data) => analyzeForTemplate(data, template);

  const handleDocxPopulate = async (imported) => {
    // The document is authored for one specific template, and the template
    // decides which fields exist and how Industries behave. Resolve and apply
    // it FIRST so every content decision below uses the final template.
    // handleTemplateChange is deliberately NOT used here: it clears the
    // country link, which the document may have just resolved.
    const targetTemplate = normalizeTemplate(imported.template || template);
    if (imported.template) setTemplate(targetTemplate);

    const importSetters = {
      ...buildSuggestionSetters(),
      setSlug,
      setSlugEdited,
      setCountry,
      setMarket,
    };
    applySbcSeoSuggestion(imported, importSetters, uid);
    if (imported.city) setCity(imported.city);
    if (imported.citySlug) {
      setCitySlug(imported.citySlug);
      setCitySlugEdited(true);
    }
    if (imported.countryRef) setCountryRef(imported.countryRef);

    // "What We Do" stage names are fixed CMS values, and the edit form
    // re-matches saved steps against that fixed list by shortTitle. Importing
    // the document's own stage titles makes that match fail, so opening and
    // re-saving the page blanks the whole section. Import only the editable
    // content and keep the CMS stage names.
    const importedSteps = imported.whatWeDo?.steps;
    if (Array.isArray(importedSteps) && importedSteps.length) {
      setWhatWeDoSteps((prev) =>
        mergeFixedStageSteps(prev.length ? prev : buildStaticSteps(), importedSteps)
      );
    }

    // applySbcSeoSuggestion's industries branch carries only name/points/
    // ctaLink/iconAlt — it drops `sub` (the Description editor) and `icon`.
    // That is fine for the AI flow, which doesn't generate them, but the
    // document does, so re-apply the industries rows here with the full set
    // rather than changing shared AI behaviour.
    const items = imported.industries?.items;
    if (Array.isArray(items) && items.length) {
      if (industriesAreFreeForm(targetTemplate)) {
        // v2 templates: free-form list. Icon/image/link are derived from the
        // name exactly as typing it into the form would.
        const options = await getIndustryTableData({ limit: 200, page: 1 })
          .then(({ items: rows }) =>
            (rows || [])
        ...
        // Default template: the taxonomy is fixed and its Name input is
        // readOnly, so replacing names here would leave the admin with values
        // they cannot correct. Merge only the editable bullet points.
        setIndustries((prev) =>
          mergeFixedTaxonomyIndustries(prev.length ? prev : buildStaticIndustries(), items)
        );
      }
    }
  };
  ...
  const buildRestoreSnapshot = () => cloneFormValue({
    ...
    caseStudiesShow, caseStudiesHeading, caseStudiesSubtitle, caseStudiesFeatured, caseStudiesOthers,
    testimonialsShow,
    blogShow, blogHeading, blogSubtitle, blogSelected,
  ...
  const applyRestoreSnapshot = (snapshot) => {
    ...
    setBlogSelected(snapshot.blogSelected || ["", "", ""]);
  ...
  const buildAiPayload = () => {
    ...
    return {
      ...
      ...formAwareBrief,
      targetCity: core.city || formAwareBrief.targetCity,
    };
  };
  ...
  const handleGenerateSeoSuggestions = async (group = null) => {
    ...
    try {
      ...
      const data = await generateSBCCitySeoSuggestionsAPI(groups ? { ...payload, groups } : payload);
      ...
      // Surface the backend's real message — it flags static-template fallback and
      // sibling-overlap review warnings that must not be silently published.
  ...
  const handleSubmit = async (e) => {
    ...
    if (!countryRef) newErrors.countryRef = "Country page is required";
    ...
    try {
      ...
      const plain = {
        title, slug, countryRef, city, citySlug, status, isDraft,
        template,
        ...
        processShow, processHeading, processIntro, processCtaText, processCtaLink,
        ...
        caseStudiesShow, caseStudiesHeading, caseStudiesSubtitle,
        testimonialsShow, testimonialsHeading, blogShow, blogHeading, blogSubtitle,
      ...
      // Why location image
      if (whyImage) fd.append("whyImage", whyImage);
      ...
      // Blog card picks (empty = auto-fill with latest published)
      blogSelected.filter(Boolean).forEach((bId, i) => fd.append(`selectedBlogs[${i}]`, bId));
      ...
      const data = await addServiceByCityAPI(fd);
      toast.success(data.message || "Created successfully");
      if (data.status === "success") {
        const doc = data.data;
        setSavedDocId(doc?._id || null);
        const secret = process.env.NEXT_PUBLIC_PREVIEW_SECRET || "";
        if (market && citySlug && slug && secret) {
          await fetch("/api/revalidate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ path: `/${market}/${citySlug}/${slug}`, secret }),
          }).catch(() => {});
        }
        if (isPreviewing) {
          const editPath = doc?._id ? `/thebusinesshub/service-by-city/edit/${doc._id}` : "";
          const redirectTo = `/${market}/${citySlug}/${slug}`;
          const params = new URLSearchParams({ secret, redirect: redirectTo });
          if (editPath) params.set("edit", editPath);
          const previewUrl = `${window.location.origin}/api/preview/enable?${params.toString()}`;
          // No "noopener" in the features string: per spec that makes window.open
          // return null even on SUCCESS, so the popup-blocked fallback below would
          // ALSO navigate this tab to the preview. Open normally and null the
          // opener manually instead (same as the country-page flow).
          const opened = window.open(previewUrl, "_blank");
          if (opened) {
            try { opened.opener = null; } catch {}
            if (editPath) router.replace(editPath);
          } else {
          ...
          setTimeout(() => router.push("/thebusinesshub/service-by-city"), 1500);
        }
      }
      setError({});
    } catch (err) {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} onFocusCapture={fieldAiTools.onFocusCapture} className="row">
        ...
        <DocxImportPanel
          importFn={importServiceByCityDocxAPI}
          onPopulate={handleDocxPopulate}
          hasExistingData={hasExistingCityFormData}
          analyze={docxTemplateWarnings}
        />
        ...
        <SectionCard id="sec-core" title="Core Information" accentColor="#4b4d7c">
            ...
            <div className="d-flex flex-wrap align-items-end gap-2 mb20" style={{ border: "1px solid #dde2f3", borderRadius: 10, padding: 14, background: "#f8f9ff" }}>
              <div style={{ minWidth: 280, flex: "1 1 320px" }}>
                ...
                <select className="form-control" value={seoBriefKey} onChange={(e) => setSeoBriefKey(e.target.value)} disabled={isGeneratingSeo}>
                  <option value="auto">Auto-detect from title, city, or market</option>
              ...
              <span style={{ fontSize: 12, color: "#666", maxWidth: 520 }}>
                Uses the Akoode SEO landing page master prompt in city mode. URL structure (slug, market, city slug) is preserved automatically.
              </span>
            </div>
          ...
          <div className="col-lg-4">
            <div className="my_profile_setting_input form-group">
              <label>Page Title <span style={{ fontSize: 10, color: "#888", fontWeight: 400 }}>— auto-generated, editable</span></label>
              <input type="text" className="form-control" value={title} onChange={handleTitleChange} placeholder="e.g. Software Development Company London" />
          ...
          <div className="col-lg-4">
            <div className="my_profile_setting_input form-group">
              ...
              <input type="text" className="form-control" value={slug} onChange={handleSlugChange} placeholder="e.g. software-development-company-london" />
              {error.slug && <span className="text-danger">{error.slug}</span>}
            </div>
          </div>
          <div className="col-lg-4">
            <div className="my_profile_setting_input form-group">
              <label>Country Page <span style={{ fontSize: 10, color: "#888", fontWeight: 400 }}>— link to existing country page</span></label>
              <select className="form-control" value={countryRef} onChange={handleCountrySelect}>
                <option value="">— Select Country Page —</option>
                {filteredSbcOptions.map(opt => (
                  <option key={opt._id} value={opt._id}>{opt.market}</option>
                ))}
              </select>
              {error.countryRef && <span className="text-danger">{error.countryRef}</span>}
              {countryRef && market && (
                <small style={{ color: "#888", display: "block", marginTop: 4 }}>Market: <strong>{market}</strong></small>
              )}
            </div>
          </div>
          <div className="col-lg-4">
            <div className="my_profile_setting_input form-group">
              ...
              <input type="text" className="form-control" value={city} onChange={handleCityChange} placeholder="e.g. London" />
              {error.city && <span className="text-danger">{error.city}</span>}
            </div>
          </div>
          <div className="col-lg-4">
            <div className="my_profile_setting_input form-group">
              <label>City Slug <span style={{ fontSize: 10, color: "#888", fontWeight: 400 }}>— auto-generated, editable</span></label>
              <input type="text" className="form-control" value={citySlug} onChange={handleCitySlugChange} placeholder="e.g. london" />
            </div>
          </div>
          {market && citySlug && slug && (
            <div className="col-lg-12">
              <small style={{ color: "#888", fontSize: 12 }}>
                Public URL: <strong>/{market}/{citySlug}/{slug}</strong>
              </small>
            </div>
          )}
        </SectionCard>
            ...
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  ...
                  <input type="text" className="form-control" value={heroHeading} onChange={e => setHeroHeading(e.target.value)} placeholder="e.g. Software Development in London" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
              ...
              {/* Fields differ by Page Template — see the "Page Template"
                  selector in Core Info. MadLocationHero uses a fixed template
                  illustration and four editable stat cards (CTA text/link are
                  fixed, not editable); EcomLocationHero uses editable
                  primary/secondary CTA buttons AND four editable stat cards;
                  AiLocationHero uses those same CTA buttons but reads the
                  shared `heroStats` array as a five-slot trust bar instead —
                  `value` is the bold line, `sub` the muted one, `label` unused.
                  Hence usesHeroStats and usesHeroBadges being separate gates
                  over one piece of state. */}
              {usesV2Sections(template) ? (
                <>
                  {/* CTA fields — every v2 template except MAD (whose CTA
                      text/link are fixed, not admin-editable). Ecommerce
                      needs these AND the stats below, so this can't be an
                      either/or with usesHeroStats. */}
              ...
              <div className="col-lg-4">
                ...
                <UploadWithAlt
                  ...
                  onRemove={() => { setHeroImageFile(null); setHeroImagePreview(""); }}
                >
            ...
            <>
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  ...
                  <input type="text" className="form-control" value={chooseUsHeading} onChange={e => setChooseUsHeading(e.target.value)} placeholder="e.g. Built with Precision, Validated by Results." />
                </div>
              </div>
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  <label>Intro Paragraph</label>
                  <HtmlEditor value={chooseUsIntro} onChange={(v) => setChooseUsIntro(v)} placeholder="Short intro paragraph" />
                </div>
              </div>
                ...
                <div className="col-12" key={f.id}>
                  <StepCard index={i} label="Feature" onRemove={() => removeFromList(setChooseUsFeatures, f.id)}>
                    <div className="col-xl-3">
                      <IconPicker value={f.icon} onChange={v => updateListItem(setChooseUsFeatures, f.id, "icon", v)} />
                    </div>
                    <div className="col-xl-4">
                      <div className="my_profile_setting_input form-group">
                        <label>Title</label>
                        <input type="text" className="form-control" value={f.title} onChange={e => updateListItem(setChooseUsFeatures, f.id, "title", e.target.value)} />
                      </div>
                    </div>
                    <div className="col-xl-5">
                      <div className="my_profile_setting_input form-group">
                        <label>Body</label>
                        <HtmlEditor value={f.body} onChange={(v) => updateListItem(setChooseUsFeatures, f.id, "body", v)} />
                      </div>
                    </div>
                  </StepCard>
                </div>
              ...
              {clientLove.map((c, i) => (
                <div className="col-12" key={c.id}>
                  <StepCard index={i} label="Client Love" onRemove={() => removeFromList(setClientLove, c.id)}>
                    <div className="col-xl-3">
                      <IconPicker value={c.icon} onChange={v => updateListItem(setClientLove, c.id, "icon", v)} />
                    </div>
                    <div className="col-xl-3">
                      <div className="my_profile_setting_input form-group">
                        <label>Title</label>
                        <input type="text" className="form-control" value={c.title} onChange={e => updateListItem(setClientLove, c.id, "title", e.target.value)} placeholder="e.g. On-time Delivery" />
                      </div>
                    </div>
                    <div className="col-xl-6">
                      <div className="my_profile_setting_input form-group">
                        <label>Body</label>
                        <input type="text" className="form-control" value={c.body} onChange={e => updateListItem(setClientLove, c.id, "body", e.target.value)} placeholder="e.g. 98% of projects delivered on time" />
                      </div>
                    </div>
                  </StepCard>
                </div>
          ...
          {whyShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={whyHeading} onChange={e => setWhyHeading(e.target.value)} placeholder="e.g. Why London is a Leading Tech Hub…" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Paragraph 1</label>
                  <HtmlEditor value={whyPara1} onChange={(v) => setWhyPara1(v)} />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Paragraph 2</label>
                  <HtmlEditor value={whyPara2} onChange={(v) => setWhyPara2(v)} />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Paragraph 3</label>
                  <HtmlEditor value={whyPara3} onChange={(v) => setWhyPara3(v)} />
                </div>
              </div>

              <div className="col-lg-12">
                <div style={{ fontWeight: 600, marginBottom: 8, color: "#2c2e50" }}>Feature Cards</div>
              ...
              {whyFeatures.map((f, i) => (
                <div className="col-12" key={f.id}>
                  <StepCard index={i} label="Feature" onRemove={() => removeFromList(setWhyFeatures, f.id)}>
                    <div className="col-xl-3">
                      <IconPicker value={f.icon} onChange={v => updateListItem(setWhyFeatures, f.id, "icon", v)} />
                    </div>
                    <div className="col-xl-3">
                      <div className="my_profile_setting_input form-group">
                        <label>Title</label>
                        <input type="text" className="form-control" value={f.title} onChange={e => updateListItem(setWhyFeatures, f.id, "title", e.target.value)} />
                      </div>
                    </div>
                    <div className="col-xl-6">
                      <div className="my_profile_setting_input form-group">
                        <label>Body</label>
                        <HtmlEditor value={f.body} onChange={(v) => updateListItem(setWhyFeatures, f.id, "body", v)} />
                      </div>
                    </div>
                  </StepCard>
                </div>
              ...
              <div className="col-lg-6">
                ...
                <UploadWithAlt
                  ...
                  altValue={whyImageAlt}
                  altOnChange={setWhyImageAlt}
                  altPlaceholder="Describe the location image"
                  hasFile={!!whyImagePreview}
                  onRemove={() => { setWhyImage(null); setWhyImagePreview(""); }}
                >
                  <div className="wrap-custom-file">
                    <input type="file" id="whyImage" accept="image/*" onChange={uploadWhyImage} />
                    <label style={whyImagePreview ? { backgroundImage: `url(${whyImagePreview})` } : {}} htmlFor="whyImage">
              ...
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  <label>Card Footer — Location Label</label>
                  <input type="text" className="form-control" value={whyCardLocation} onChange={e => setWhyCardLocation(e.target.value)} placeholder="e.g. Gurugram, India" />
                </div>
                <div className="my_profile_setting_input form-group">
                  <label>Card Footer — Heading</label>
                  <input type="text" className="form-control" value={whyCardHeading} onChange={e => setWhyCardHeading(e.target.value)} placeholder="e.g. India's Global Capability Hub" />
                </div>
                <div className="my_profile_setting_input form-group">
                  <label>Card Footer — Body</label>
                  <HtmlEditor value={whyCardBody} onChange={(v) => setWhyCardBody(v)} placeholder="Short tagline" />
                </div>
              </div>
            </>
          )}
          ...
          {processShow && (
            <>
              <div className="col-lg-8">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={processHeading} onChange={e => setProcessHeading(e.target.value)} placeholder="e.g. Five disciplines, one team under one roof." />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Intro</label>
                  <HtmlEditor value={processIntro} onChange={(v) => setProcessIntro(v)} placeholder="Short paragraph below the section heading" />
                </div>
              </div>
              <div className="col-lg-2">
                <div className="my_profile_setting_input form-group">
                  <label>CTA Button Text</label>
                  <input type="text" className="form-control" value={processCtaText} onChange={e => setProcessCtaText(e.target.value)} placeholder="e.g. Start your project" />
                </div>
              </div>
              <div className="col-lg-2">
                <div className="my_profile_setting_input form-group">
                  <label>CTA Button Link</label>
                  <input type="text" className="form-control" value={processCtaLink} onChange={e => setProcessCtaLink(e.target.value)} placeholder="e.g. /post-requirement" />
                </div>
              </div>
              <div className="col-lg-12">
                <button type="button" className="btn admore_btn mb20" onClick={() => addToList(setProcessServices, emptyProcessService)}>+ Add Service Card</button>
              </div>
              {processServices.map((s, i) => (
                <div className="col-12" key={s.id}>
                  <StepCard index={i} label="Service" onRemove={() => removeFromList(setProcessServices, s.id)}>
                    <div className="col-xl-2">
                      <div className="my_profile_setting_input form-group">
                        <label>Number</label>
                        <input type="text" className="form-control" value={s.n} onChange={e => updateListItem(setProcessServices, s.id, "n", e.target.value)} placeholder="01" />
                      </div>
                    </div>
                    {usesV2Sections(template) ? (
                      ...
                      <>
                        <div className="col-xl-4">
                          <div className="my_profile_setting_input form-group">
                            <label>Title</label>
                            <input type="text" className="form-control" value={s.title} onChange={e => updateListItem(setProcessServices, s.id, "title", e.target.value)} placeholder="e.g. Web Platforms" />
                          </div>
                        </div>
                        <div className="col-xl-6">
                          <div className="my_profile_setting_input form-group">
                            <label>Subtitle</label>
                            <input type="text" className="form-control" value={s.subtitle} onChange={e => updateListItem(setProcessServices, s.id, "subtitle", e.target.value)} placeholder="Short subtitle" />
                          </div>
                        </div>
                      </>
                    )}
                    <div className="col-xl-12">
                      <div className="my_profile_setting_textarea form-group">
                        ...
                        <HtmlEditor value={s.para} onChange={(v) => updateListItem(setProcessServices, s.id, "para", v)} />
                      </div>
                    </div>
                    <div className="col-xl-6">
                      ...
                      {s.points.map((pt, j) => (
                        <div key={j} className="d-flex gap-2 mb-2">
                          <input type="text" className="form-control" value={pt} onChange={e => updateSubItem(setProcessServices, s.id, "points", j, e.target.value)} placeholder={`Point ${j + 1}`} />
                    ...
                    <div className="col-xl-6">
                      ...
                      {s.tags.map((tg, j) => (
                        <div key={j} className="d-flex gap-2 mb-2">
                          <input type="text" className="form-control" value={tg} onChange={e => updateSubItem(setProcessServices, s.id, "tags", j, e.target.value)} placeholder={`Tag ${j + 1}`} />
        ...
        {/* ── What We Do ────────────────────────────────────────────────────── */}
        <SectionCard id="sec-whatwedo" title="What we do" accentColor="#3c3e66" toggleId="whatWeDoShow" toggleName="whatWeDoShow" enabled={whatWeDoShow} onToggle={e => { setWhatWeDoShow(e.target.checked); if (e.target.checked && whatWeDoSteps.length === 0) setWhatWeDoSteps(buildStaticSteps()); }} badge={whatWeDoSteps.length}>
          {whatWeDoShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={whatWeDoHeading} onChange={e => setWhatWeDoHeading(e.target.value)} placeholder="e.g. Everything your product needs, under one roof." />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={whatWeDoSubtitle} onChange={(v) => setWhatWeDoSubtitle(v)} placeholder="Short paragraph below the section heading" />
                </div>
              </div>
              <div className="col-lg-12">
                <button type="button" className="btn admore_btn mb20" onClick={() => addToList(setWhatWeDoSteps, emptyWhatWeDoStep)}>+ Add Step</button>
              </div>
              {whatWeDoSteps.map((s, i) => (
                <div className="col-12" key={s.id}>
                  <StepCard index={i} label="Step" onRemove={() => removeFromList(setWhatWeDoSteps, s.id)}>
                    <div className="col-xl-3">
                      <div className="my_profile_setting_input form-group">
                        <label>Short Title (nav)</label>
                        <input type="text" className="form-control" value={s.shortTitle} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "shortTitle", e.target.value)} placeholder="e.g. Discovery" />
                      </div>
                    </div>
                    <div className="col-xl-4">
                      <div className="my_profile_setting_input form-group">
                        <label>Full Title</label>
                        <input type="text" className="form-control" value={s.title} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "title", e.target.value)} placeholder="e.g. Discovery & Requirements" />
                      </div>
                    </div>
                    <div className="col-xl-2">
                      <div className="my_profile_setting_input form-group">
                        <label>Timeline</label>
                        <input type="text" className="form-control" value={s.timeline} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "timeline", e.target.value)} placeholder="2 – 4 weeks" />
                      </div>
                    </div>
                    <div className="col-xl-3">
                      <div className="my_profile_setting_input form-group">
                        <label>Timeline Note</label>
                        <input type="text" className="form-control" value={s.timelineNote} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "timelineNote", e.target.value)} placeholder="From signed scope to…" />
                      </div>
                    </div>
                    <div className="col-xl-12">
                      <div className="my_profile_setting_textarea form-group">
                        <label>Body</label>
                        <HtmlEditor value={s.body} onChange={(v) => updateListItem(setWhatWeDoSteps, s.id, "body", v)} />
                      </div>
                    </div>
                    <div className="col-xl-12">
                      ...
                      {s.deliverables.map((d, j) => (
                        <div key={j} className="d-flex gap-2 mb-2">
                          <input type="text" className="form-control" value={d} onChange={e => updateSubItem(setWhatWeDoSteps, s.id, "deliverables", j, e.target.value)} placeholder={`Deliverable ${j + 1}`} />
        ...
        {/* ── Tech Stack — static for the default template (TechStack.jsx);
              editable for v2 templates, which render it via MadTechnologies.
              Editor is the Updated Services one: a logo picker per tab, not
              per-logo uploads. Stored as techStack.cats[].pills[]. ────────── */}
        {usesV2Sections(template) && (
        ...
        {/* ── Industries ────────────────────────────────────────────────────── */}
        <SectionCard id="sec-industries" title="Industries Section" accentColor="#4b4d7c" toggleId="industriesShow" toggleName="industriesShow" enabled={industriesShow} onToggle={e => { setIndustriesShow(e.target.checked); if (e.target.checked && industries.length === 0 && !usesV2Sections(template)) setIndustries(buildStaticIndustries()); }} badge={industries.length}>
          {industriesShow && (
            /* Fields differ by Page Template — see the "Page Template" selector
               in Core Info. v2 templates use the Updated Services editor
               (free-form list with icon/image/link pickers); the default
               template keeps the fixed shared taxonomy. */
            usesV2Sections(template) ? (
              <V2IndustriesFields
              ...
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={industriesHeading} onChange={e => setIndustriesHeading(e.target.value)} placeholder="e.g. Across every high-growth vertical." />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={industriesSubtitle} onChange={(v) => setIndustriesSubtitle(v)} placeholder="Short paragraph below the industries heading" />
                </div>
              </div>

              {industries.map((ind, i) => (
                <div className="col-12" key={ind.id}>
                  <StepCard index={i} label={ind.name}>
                    <div className="col-xl-12">
                      <label style={{ fontWeight: 600 }}>Pointers</label>
                      {ind.points.map((pt, j) => (
                        <div key={j} className="d-flex gap-2 mb-2">
                      ...
                      <button type="button" className="btn admore_btn btn-sm mt-1" onClick={() => addSubItem(setIndustries, ind.id, "points", "")}>+ Pointer</button>
                    </div>
                  </StepCard>
                </div>
              ))}
          ...
          {engagementShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={engagementHeading} onChange={e => setEngagementHeading(e.target.value)} placeholder="e.g. Three ways to work with us." />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={engagementSubtitle} onChange={(v) => setEngagementSubtitle(v)} placeholder="Short paragraph below the engagement heading" />
                </div>
              </div>
            </>
          )}
          ...
          {faqShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={faqHeading} onChange={e => setFaqHeading(e.target.value)} placeholder="e.g. Everything teams ask us first." />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={faqSubtitle} onChange={(v) => setFaqSubtitle(v)} placeholder="Short paragraph below the FAQ heading" />
                </div>
              </div>
              <div className="col-lg-12">
                <button type="button" className="btn admore_btn mb20" onClick={() => addToList(setFaqs, emptyFaq)}>+ Add FAQ</button>
              </div>
              {faqs.map((faq, i) => (
                  ...
                  <StepCard index={i} label="FAQ" onRemove={() => removeFromList(setFaqs, faq.id)}>
                    <div className="col-xl-5">
                      <div className="my_profile_setting_input form-group">
                        ...
                        <input type="text" className="form-control" value={faq.question} onChange={e => updateListItem(setFaqs, faq.id, "question", e.target.value)} placeholder="Enter question" />
                      </div>
                    </div>
                    <div className="col-xl-7">
                      <div className="my_profile_setting_textarea form-group">
                        <label>Answer</label>
                        <textarea className="form-control" rows="3" value={faq.answer} onChange={e => updateListItem(setFaqs, faq.id, "answer", e.target.value)} placeholder="Enter answer" />
                      </div>
                    </div>
                  </StepCard>
                </div>
        ...
        <SectionCard id="sec-testimonials" title="Testimonials Section" accentColor="#666894" toggleId="testimonialsShow" toggleName="testimonialsShow" enabled={testimonialsShow} onToggle={e => setTestimonialsShow(e.target.checked)}>
          {/* Gated on the section toggle, like every other SectionCard, so
              turning Testimonials off hides its fields instead of leaving them
              on screen. Fields then differ by Page Template: v2 templates
              render per-page testimonials, the default pulls from the global
        ...
        <SectionCard id="sec-blog" title="Blog Section" accentColor="#666894" toggleId="blogShow" toggleName="blogShow" enabled={blogShow} onToggle={e => setBlogShow(e.target.checked)}>
          {blogShow && (
            <div className="row p-3">
              ...
              <div className="col-lg-12">
                <BlogPicker selected={blogSelected} onChange={setBlogSelected} />
              </div>
              <div className="col-lg-12">
                <p className="text-muted" style={{ fontSize: 13 }}>
                  Pick a specific blog for each of the three cards above — any slot left on Auto shows the latest active blog post.
                </p>
              </div>
            </div>
          )}
          ...
          {whyChooseShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={whyChooseHeading} onChange={e => setWhyChooseHeading(e.target.value)} placeholder="e.g. Why London Teams Choose Akoode Over Local Agencies" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={whyChooseSubtitle} onChange={(v) => setWhyChooseSubtitle(v)} placeholder="Short paragraph below the section heading" />
                </div>
              </div>

              <div className="col-lg-12">
                <div style={{ fontWeight: 600, marginBottom: 8, color: "#2c2e50" }}>Proof Cards (4 cards)</div>
              ...
              {whyChooseCards.map((card, i) => (
                <div className="col-12" key={card.id}>
                  <StepCard index={i} label="Card" onRemove={() => removeFromList(setWhyChooseCards, card.id)}>
                    <div className="col-xl-3">
                      <IconPicker value={card.icon} onChange={v => updateListItem(setWhyChooseCards, card.id, "icon", v)} label="Pick Icon" />
                    </div>
                    <div className="col-xl-4">
                      <div className="my_profile_setting_input form-group">
                        <label>Title</label>
                        <input type="text" className="form-control" value={card.title} onChange={e => updateListItem(setWhyChooseCards, card.id, "title", e.target.value)} placeholder="e.g. No Subcontracting, Ever" />
                      </div>
                    </div>
                    <div className="col-xl-5">
                      <div className="my_profile_setting_input form-group">
                        <label>Description</label>
                        <HtmlEditor value={card.desc} onChange={(v) => updateListItem(setWhyChooseCards, card.id, "desc", v)} placeholder="Card description..." />
                      </div>
                    </div>
                  </StepCard>
                </div>
          ...
          {finalCtaShow && (
            <>
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  <label>Heading</label>
                  <input type="text" className="form-control" value={finalCtaHeading} onChange={e => setFinalCtaHeading(e.target.value)} placeholder="e.g. Start your project with Akoode." />
                </div>
              </div>
              <div className="col-lg-6">
                <div className="my_profile_setting_textarea form-group">
                  <label>Body Text</label>
                  <HtmlEditor value={finalCtaBody} onChange={(v) => setFinalCtaBody(v)} placeholder="Tell us what you're building…" />
                </div>
              </div>
        ...
        <div style={{ position: "sticky", bottom: 0, zIndex: 99, background: "#fff", borderTop: "1px solid #e4e4f0", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, boxShadow: "0 -2px 10px rgba(75,77,124,0.1)", marginTop: "8px", width: "100%" }}>
          <button type="button" className="btn btn1" onClick={() => window.location.href = "/thebusinesshub/service-by-city"}>← Back</button>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                const secret = process.env.NEXT_PUBLIC_PREVIEW_SECRET || "";
                window.open(`${siteUrl}/${market}/${citySlug}/${slug}?preview=${secret}`, "_blank");
              }}
            >
              Preview
            </button>
