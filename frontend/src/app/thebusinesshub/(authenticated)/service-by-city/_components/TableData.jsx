"use client";
import { deleteSBCCityAPI, getSBCCityTableData } from "@/api/serviceByCity";
...
const TableData = () => {
  ...
  const fetchData = useCallback(async (page = 1, query = "") => {
    try {
      setLoading(true);
      const data = await getSBCCityTableData({ limit: PAGE_SIZE, page, q: query || undefined });
      setList(Array.isArray(data?.items) ? data.items : []);
      setTotalCount(data?.totalCount ?? 0);
    } catch (error) {
      console.error("Error fetching service-by-city:", error);
  ...
  const deleteEntry = async (id) => {
    ...
    try {
      const data = await deleteSBCCityAPI(id);
  ...
  const theadContent = ["Page Title", "Country/Market", "City", "Status", "Date Published", "Action"];
  ...
  const tbodyContent = list.map((item) => (
    <tr key={item._id}>
      ...
      <td>{normalizeCountryLabel(item.country) || item.market || "—"}</td>
      <td>{item.city || "—"}</td>
      <td>
      ...
      <td>
        <ul className="view_edit_delete_list mb0">
          <li className="list-inline-item" data-toggle="tooltip" data-placement="top" title="Edit">
            <button onClick={() => router.push(`/thebusinesshub/service-by-city/edit/${item._id}`)}>
          ...
          {loading ? (
            ...
            <tr>
              <td colSpan={6} className="text-center p-4">
                No entries found.{" "}
                <a href="/thebusinesshub/service-by-city/add">Add your first entry</a>
              </td>
            </tr>
          )}
        </tbody>
