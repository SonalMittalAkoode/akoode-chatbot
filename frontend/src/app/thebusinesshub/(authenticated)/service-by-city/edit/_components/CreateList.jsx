...
import { getSBCCityById, updateSBCCityAPI, generateSBCCitySeoSuggestionsAPI } from "@/api/serviceByCity";
...
const CreateList = () => {
  ...
  const initialCitySlugRef = useRef("");
  const [template, setTemplate] = useState(DEFAULT_PAGE_TEMPLATE);
  const [statusValue, setStatusValue] = useState("active");
  const [isPreviewing, setIsPreviewing] = useState(false);
  // Ref mirror of isPreviewing: the Preview button calls requestSubmit() in the
  // same tick as setIsPreviewing(true), so the submit handler's closure still
  // sees the OLD state (false) and would save without opening the preview. The
  // ref is readable synchronously — same fix as the country edit form.
  ...
  const updateIndustryIconImg = (id, file) => {
    setIndustries(p => p.map(x => {
      ...
      return { ...x, iconImg: file || null, iconImgPreview: file ? URL.createObjectURL(file) : "", iconAlt: newAlt };
    }));
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await getSBCCityById(id);
        ...
        setSlug(d.slug ?? "");
        setSlugEdited(true);
        // countryRef may be populated object or raw ObjectId string
        const refId = d.countryRef?._id ?? d.countryRef ?? "";
        setCountryRef(String(refId));
        setCountry(d.country ?? "");
        const loadedMarket = d.market ?? toSlug(d.country ?? "");
        setMarket(loadedMarket);
        const loadedCity = d.city ?? "";
        setCity(loadedCity);
        const loadedCitySlug = d.citySlug ?? toSlug(d.city ?? "");
        setCitySlug(loadedCitySlug);
        setCitySlugEdited(true);
        initialSlugRef.current = d.slug ?? "";
        initialMarketRef.current = loadedMarket;
        initialCitySlugRef.current = loadedCitySlug;
        ...
        const savedIndustries = hydrateList(ind.items).map(x => ({
          ...x,
          points: Array.isArray(x.points) && x.points.length ? x.points : (x.sub ? [x.sub] : [""]),
        }));
        const currentTemplate = normalizeTemplate(d.template);
        setIndustries(usesV2Sections(currentTemplate) ? savedIndustries : mergeWithStaticIndustries(savedIndustries));
        ...
        // selectedBlogs arrive as raw ids (or populated docs on legacy data)
        const savedBlogPicks = (blog.selectedBlogs || []).filter(Boolean).map(x => String(x._id ?? x));
        setBlogSelected([savedBlogPicks[0] || "", savedBlogPicks[1] || "", savedBlogPicks[2] || ""]);
  ...
  const buildRestoreSnapshot = () => cloneFormValue({
    ...
    testimonialsShow, testimonialsHeading, testimonialItems,
  ...
  const handleGenerateSeoSuggestions = async (group = null) => {
    ...
    try {
      ...
      // Trust the server's own list of what it produced; fall back to the request.
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await updateSBCCityAPI(id, fd);
      toast.success(data.message || "Updated successfully");
      if (data.status === "success") {
        const secret = process.env.NEXT_PUBLIC_PREVIEW_SECRET || "";
        if (secret) {
          const newPath = market && citySlug && slug ? `/${market}/${citySlug}/${slug}` : null;
          const oldPath =
            initialSlugRef.current &&
            initialMarketRef.current &&
            initialCitySlugRef.current &&
            (initialSlugRef.current !== slug || initialMarketRef.current !== market || initialCitySlugRef.current !== citySlug)
              ? `/${initialMarketRef.current}/${initialCitySlugRef.current}/${initialSlugRef.current}`
              : null;
          ...
          initialMarketRef.current = market;
          initialCitySlugRef.current = citySlug;
        }
        if (shouldPreview) {
          const editPath = `/thebusinesshub/service-by-city/edit/${id}`;
          const redirectTo = `/${market}/${citySlug}/${slug}`;
          const params = new URLSearchParams({ secret, redirect: redirectTo });
          params.set("edit", editPath);
          ...
          // return null even on SUCCESS, which made the popup-blocked fallback below
          // ALSO navigate this edit tab to the preview. Open normally and null the
          // opener manually instead (same as the country-page flow); the fallback
          // then only runs when the popup blocker genuinely intervened.
    ...
    <>
      ...
      <form onSubmit={handleSubmit} onFocusCapture={fieldAiTools.onFocusCapture} className="row">
        ...
        <SectionCard id="sec-whylocation" title="Why Location Section" accentColor="#666894" toggleId="whyShow" toggleName="whyShow" enabled={whyShow} onToggle={e => setWhyShow(e.target.checked)} badge={whyFeatures.length}>
          {whyShow && (
            <>
              <div className="col-lg-12"><div className="my_profile_setting_input form-group"><label>Section Heading</label><input type="text" className="form-control" value={whyHeading} onChange={e => setWhyHeading(e.target.value)} placeholder="e.g. Why London is a Leading Tech Hub…" /></div></div>
              ...
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group"><label>Card Footer — Location Label</label><input type="text" className="form-control" value={whyCardLocation} onChange={e => setWhyCardLocation(e.target.value)} placeholder="e.g. Gurugram, India" /></div>
                <div className="my_profile_setting_input form-group"><label>Card Footer — Heading</label><input type="text" className="form-control" value={whyCardHeading} onChange={e => setWhyCardHeading(e.target.value)} placeholder="e.g. India's Global Capability Hub" /></div>
                <div className="my_profile_setting_input form-group"><label>Card Footer — Body</label><HtmlEditor value={whyCardBody} onChange={(v) => setWhyCardBody(v)} placeholder="Short tagline" /></div>
              </div>
            </>
          )}
        </SectionCard>
        ...
        <SectionCard id="sec-process" title="Services Offered" accentColor="#4b4d7c" toggleId="processShow" toggleName="processShow" enabled={processShow} onToggle={e => setProcessShow(e.target.checked)} badge={processServices.length}>
          {processShow && (
            <>
              ...
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Intro</label><HtmlEditor value={processIntro} onChange={(v) => setProcessIntro(v)} /></div></div>
                ...
                <div className="col-12" key={s.id}>
                  <StepCard index={i} label="Service" onRemove={() => removeFromList(setProcessServices, s.id)}>
                    ...
                    <div className="col-xl-6">
                      ...
                      {s.points.map((pt, j) => (<div key={j} className="d-flex gap-2 mb-2"><input type="text" className="form-control" value={pt} onChange={e => updateSubItem(setProcessServices, s.id, "points", j, e.target.value)} /><button type="button" className="btn btn-sm btn-outline-danger" onClick={() => removeSubItem(setProcessServices, s.id, "points", j)}>×</button></div>))}
                    ...
                    <div className="col-xl-6">
                      ...
                      {s.tags.map((tg, j) => (<div key={j} className="d-flex gap-2 mb-2"><input type="text" className="form-control" value={tg} onChange={e => updateSubItem(setProcessServices, s.id, "tags", j, e.target.value)} /><button type="button" className="btn btn-sm btn-outline-danger" onClick={() => removeSubItem(setProcessServices, s.id, "tags", j)}>×</button></div>))}
        ...
        {/* ── What We Do ────────────────────────────────────────────────────── */}
        <SectionCard id="sec-whatwedo" title="What we do" accentColor="#3c3e66" toggleId="whatWeDoShow" toggleName="whatWeDoShow" enabled={whatWeDoShow} onToggle={e => { setWhatWeDoShow(e.target.checked); if (e.target.checked && whatWeDoSteps.length === 0) setWhatWeDoSteps(mergeWithStaticSteps([])); }} badge={whatWeDoSteps.length}>
          {whatWeDoShow && (
            <>
              ...
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Subtitle</label><HtmlEditor value={whatWeDoSubtitle} onChange={(v) => setWhatWeDoSubtitle(v)} /></div></div>
              <div className="col-lg-12">
              ...
              {whatWeDoSteps.map((s, i) => (
                <div className="col-12" key={s.id}>
                  <StepCard index={i} label="Step" onRemove={() => removeFromList(setWhatWeDoSteps, s.id)}>
                    <div className="col-xl-3"><div className="my_profile_setting_input form-group"><label>Short Title (nav)</label><input type="text" className="form-control" value={s.shortTitle} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "shortTitle", e.target.value)} placeholder="e.g. Discovery" /></div></div>
                    <div className="col-xl-4"><div className="my_profile_setting_input form-group"><label>Full Title</label><input type="text" className="form-control" value={s.title} onChange={e => updateListItem(setWhatWeDoSteps, s.id, "title", e.target.value)} placeholder="e.g. Discovery & Requirements" /></div></div>
                    ...
                    <div className="col-xl-12">
                      ...
                      {s.deliverables.map((d, j) => (<div key={j} className="d-flex gap-2 mb-2"><input type="text" className="form-control" value={d} onChange={e => updateSubItem(setWhatWeDoSteps, s.id, "deliverables", j, e.target.value)} /><button type="button" className="btn btn-sm btn-outline-danger" onClick={() => removeSubItem(setWhatWeDoSteps, s.id, "deliverables", j)}>×</button></div>))}
        ...
        <SectionCard id="sec-industries" title="Industries Section" accentColor="#4b4d7c" toggleId="industriesShow" toggleName="industriesShow" enabled={industriesShow} onToggle={e => { setIndustriesShow(e.target.checked); if (e.target.checked && industries.length === 0 && !usesV2Sections(template)) setIndustries(mergeWithStaticIndustries([])); }} badge={industries.length}>
          {industriesShow && (
            ...
            usesV2Sections(template) ? (
              ...
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Subtitle</label><HtmlEditor value={industriesSubtitle} onChange={(v) => setIndustriesSubtitle(v)} /></div></div>
                ...
                <div className="col-12" key={ind.id}>
                  <StepCard index={i} label={ind.name}>
                    <div className="col-xl-12">
                      ...
                      {ind.points.map((pt, j) => (<div key={j} className="d-flex gap-2 mb-2"><input type="text" className="form-control" value={pt} onChange={e => updateSubItem(setIndustries, ind.id, "points", j, e.target.value)} placeholder={`Pointer ${j + 1}`} /><button type="button" className="btn btn-sm btn-outline-danger" onClick={() => removeSubItem(setIndustries, ind.id, "points", j)}>×</button></div>))}
        ...
        <SectionCard id="sec-engagement" title="Engagement Models Section" accentColor="#3c3e66" toggleId="engagementShow" toggleName="engagementShow" enabled={engagementShow} onToggle={e => setEngagementShow(e.target.checked)}>
          {engagementShow && (
            <>
              ...
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Subtitle</label><HtmlEditor value={engagementSubtitle} onChange={(v) => setEngagementSubtitle(v)} /></div></div>
            </>
          )}
        </SectionCard>
        ...
        <SectionCard id="sec-faq" title="FAQ Section" accentColor="#4b4d7c" toggleId="faqShow" toggleName="faqShow" enabled={faqShow} onToggle={e => setFaqShow(e.target.checked)} badge={faqs.length}>
          {faqShow && (
            <>
              ...
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Subtitle</label><HtmlEditor value={faqSubtitle} onChange={(v) => setFaqSubtitle(v)} /></div></div>
        ...
        <SectionCard id="sec-whychoose" title="Why Choose Akoode Section" accentColor="#3c3e66" toggleId="whyChooseShow" toggleName="whyChooseShow" enabled={whyChooseShow} onToggle={e => setWhyChooseShow(e.target.checked)} badge={whyChooseCards.length}>
          {whyChooseShow && (
            <>
              <div className="col-lg-12"><div className="my_profile_setting_input form-group"><label>Section Heading</label><input type="text" className="form-control" value={whyChooseHeading} onChange={e => setWhyChooseHeading(e.target.value)} /></div></div>
              <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Section Subtitle</label><HtmlEditor value={whyChooseSubtitle} onChange={(v) => setWhyChooseSubtitle(v)} /></div></div>
              <div className="col-lg-12">
        ...
        <SectionCard id="sec-meta" title="Meta Information" accentColor="#A3B18A">
          <div className="col-lg-12"><div className="my_profile_setting_input form-group"><label>Meta Title</label><input type="text" className="form-control" value={metaTitle} onChange={e => setMetaTitle(e.target.value)} />{error.metaTitle && <span className="text-danger">{error.metaTitle}</span>}</div></div>
          <div className="col-lg-12"><div className="my_profile_setting_textarea form-group"><label>Meta Description</label><textarea className="form-control" rows="4" value={metaDescription} onChange={e => setMetaDescription(e.target.value)} />{error.metaDescription && <span className="text-danger">{error.metaDescription}</span>}</div></div>
        </SectionCard>
        ...
        <div style={{ position: "sticky", bottom: 0, zIndex: 99, background: "#fff", borderTop: "1px solid #e4e4f0", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, boxShadow: "0 -2px 10px rgba(75,77,124,0.1)", marginTop: "8px", width: "100%" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <select className="form-control" style={{ width: 130, height: 40 }} value={statusValue} onChange={e => setStatusValue(e.target.value)}>
              <option value="active">Active</option>
              <option value="draft">Draft</option>
              <option value="inactive">Inactive</option>
            </select>
            <button type="button" className="btn btn1" disabled={isSubmitting} onClick={() => { if (statusValue !== "active") setStatusValue("draft"); setIsPreviewing(false); document.querySelector("form")?.requestSubmit(); }}>Save as Draft</button>
            <button type="button" className="btn btn1" disabled={isSubmitting} onClick={() => { previewSubmitRef.current = true; setIsPreviewing(true); document.querySelector("form")?.requestSubmit(); }}>{isSubmitting && isPreviewing ? "Preparing Preview..." : "Preview"}</button>
            <button type="submit" className="btn btn2" disabled={isSubmitting}>{isSubmitting ? "Saving..." : "Update"}</button>
          </div>
        </div>
