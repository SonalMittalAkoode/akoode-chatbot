"use client";
import { deleteSBCAPI, getSBCTableData } from "@/api/serviceByCountry";
...
const TableData = () => {
  ...
  const fetchData = useCallback(async (page = 1, query = "") => {
    try {
      setLoading(true);
      const data = await getSBCTableData({ limit: PAGE_SIZE, page, q: query || undefined });
      setList(Array.isArray(data?.items) ? data.items : []);
      setTotalCount(data?.totalCount ?? 0);
    } catch (error) {
      console.error("Error fetching service-by-country:", error);
  ...
  const deleteEntry = async (id) => {
    ...
    try {
      const data = await deleteSBCAPI(id);
  ...
  const theadContent = ["Page Title", "Country/Market", "Date Published", "Status", "Action"];
      ...
      <td>
        <ul className="view_edit_delete_list mb0">
          <li className="list-inline-item" data-toggle="tooltip" data-placement="top" title="Edit">
            <button onClick={() => router.push(`/thebusinesshub/service-by-country/edit/${item._id}`)}>
          ...
          {loading ? (
            ...
            <tr>
              <td colSpan={5} className="text-center p-4">
                No entries found.{" "}
                <a href="/thebusinesshub/service-by-country/add">Add your first entry</a>
              </td>
            </tr>
          )}
        </tbody>
