...
import { addServiceByCountryAPI, generateSBCSeoSuggestionsAPI } from "@/api/serviceByCountry";
import { importServiceByCountryDocxAPI } from "@/api/docxImport";
...
const buildStaticIndustries = () =>
  STATIC_INDUSTRIES.map(name => ({ id: uid(), iconImg: null, iconImgPreview: "", iconAlt: "", name, points: [""], ctaLink: "" }));
...
const emptyIndustry = () => ({ id: uid(), iconImg: null, iconImgPreview: "", iconAlt: "", name: "", points: [""], ctaLink: "" });
...
// ─── sub-array helpers (points / tags / deliverables / perks / pills) ───────
...
const CreateList = () => {
  ...
  const [statusValue, setStatusValue] = useState("draft"); // new pages start as draft
  ...
  const [industries, setIndustries] = useState(buildStaticIndustries());
  ...
  const buildSuggestionSetters = () => ({
    ...
    setTestimonialsHeading: () => {}, setTestimonialsSubtitle: () => {}, setBlogShow, setBlogHeading, setBlogSubtitle,
    setFinalCtaShow, setFinalCtaHeading, setFinalCtaBody, setFinalCtaReplyTime, setFinalCtaNda,
    setMetaTitle, setMetaDescription,
  });

  // ── DOCX import ────────────────────────────────────────────────────────────
  const hasExistingCountryFormData = Boolean(
    title.trim() || heroHeading.trim() || country.trim() || processServices.length || faqs.length
  );
  ...
  const handleDocxPopulate = async (imported) => {
    ...
    applySbcSeoSuggestion(imported, buildSuggestionSetters(), uid);
    if (imported.keywords) setCoreKeywords(imported.keywords);
  ...
  const buildExistingContentContext = () => ({
    ...
    caseStudies: { heading: caseStudiesHeading, subtitle: caseStudiesSubtitle },
    blog: { heading: blogHeading, subtitle: blogSubtitle },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const plain = {
        ...
        testimonialsShow, testimonialsHeading,
        blogShow, blogHeading, blogSubtitle,
      ...
      const data = await addServiceByCountryAPI(fd);
      toast.success(data.message || "Created successfully");
      if (data.status === "success") {
        const doc = data.data;
        setSavedDocId(doc?._id || null);
        // Revalidate the public page immediately
        const secret = process.env.NEXT_PUBLIC_PREVIEW_SECRET || "";
        if (core.market && core.slug && secret) {
          await fetch("/api/revalidate", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ path: `/${core.market}/${core.slug}`, secret }),
          }).catch(() => {});
        }
        if (shouldPreview) {
          const editPath = doc?._id ? `/thebusinesshub/service-by-country/edit/${doc._id}` : "";
          const opened = openPreviewUrl(buildPreviewUrl({ market: core.market, slug: core.slug, editPath }), editPath);
          setIsPreviewing(false);
          previewSubmitRef.current = false;
          if (opened && editPath) router.replace(editPath);
        } else {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} onFocusCapture={fieldAiTools.onFocusCapture} className="row">
        ...
        <DocxImportPanel
          importFn={importServiceByCountryDocxAPI}
          onPopulate={handleDocxPopulate}
          hasExistingData={hasExistingCountryFormData}
            ...
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  ...
                  <input type="text" className="form-control" value={heroHeading} onChange={e => setHeroHeading(e.target.value)} placeholder="e.g. Software Development" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
          ...
          {whyShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={whyHeading} onChange={e => setWhyHeading(e.target.value)} placeholder="e.g. Why the UK is Europe's Most Demanding Market…" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
                  ...
                  <StepCard index={i} label="Service" onRemove={() => removeFromList(setProcessServices, s.id)}>
                    ...
                    <div className="col-xl-6">
                      <div className="my_profile_setting_input form-group">
                        ...
                        <input type="text" className="form-control" value={s.ctaText} onChange={e => updateListItem(setProcessServices, s.id, "ctaText", e.target.value)} placeholder="e.g. Start your project" />
                      </div>
                    </div>
                    <div className="col-xl-6">
                      <div className="my_profile_setting_input form-group">
                        <label>CTA Button Link</label>
                        <input type="text" className="form-control" value={s.ctaLink} onChange={e => updateListItem(setProcessServices, s.id, "ctaLink", e.target.value)} placeholder="e.g. /post-requirement" />
                      </div>
                    </div>
                    <div className="col-xl-6">
        ...
        {/* ── What We Do (ServicesSection) ──────────────────────────────────── */}
        <SectionCard id="sec-whatwedo" title="Process Section" accentColor="#3c3e66" toggleId="whatWeDoShow" toggleName="whatWeDoShow" enabled={whatWeDoShow} onToggle={e => { setWhatWeDoShow(e.target.checked); if (e.target.checked && whatWeDoSteps.length === 0) setWhatWeDoSteps(buildStaticSteps()); }} badge={whatWeDoSteps.length}>
          {whatWeDoShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  ...
                  <StepCard index={i} label="Step">
                    <div className="col-xl-3">
                      <div className="my_profile_setting_input form-group">
                        ...
                        <input type="text" className="form-control" value={s.shortTitle} readOnly disabled style={{ background: "#f4f5fa", cursor: "not-allowed" }} />
                      </div>
                    </div>
                    <div className="col-xl-4">
                      <div className="my_profile_setting_input form-group">
                        <label>Full Title</label>
                        <input type="text" className="form-control" value={s.title} readOnly disabled style={{ background: "#f4f5fa", cursor: "not-allowed" }} />
                      </div>
                    </div>
                    <div className="col-xl-2">
                      <div className="my_profile_setting_input form-group">
        ...
        <SectionCard id="sec-industries" title="Industries Section" accentColor="#4b4d7c" toggleId="industriesShow" toggleName="industriesShow" enabled={industriesShow} onToggle={e => { setIndustriesShow(e.target.checked); if (e.target.checked && industries.length === 0 && !usesV2Sections(template)) setIndustries(buildStaticIndustries()); }} badge={industries.length}>
          {industriesShow && (
            usesV2Sections(template) ? (
              ...
              <div className="col-lg-12" style={{ background: "#f4f5fa", border: "1px dashed #c9cbe0", borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: "#474972" }}>
                Industries are <strong>fixed</strong> across all SBC pages — name and image cannot be changed. Edit the bullet points (Pointers) for each industry below.
              </div>
          ...
          {caseStudiesShow && (
            <>
              ...
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={caseStudiesSubtitle} onChange={(v) => setCaseStudiesSubtitle(v)} placeholder="Short case study section subtext" />
                </div>
              </div>
              <div className="col-lg-12">
                <CaseStudyPicker
              ...
              <div className="col-lg-12">
                <p className="text-muted" style={{ fontSize: 13, marginTop: 10 }}>Pick the featured spotlight and the two supporting cards — any slot left on Auto shows the latest published case study.</p>
              </div>
            </>
          )}
        </SectionCard>
          ...
          {blogShow && (
            <>
              ...
              <div className="col-lg-6">
                <div className="my_profile_setting_input form-group">
                  <label>Section Subtitle</label>
                  <HtmlEditor value={blogSubtitle} onChange={(v) => setBlogSubtitle(v)} placeholder="Short blog section subtext" />
                </div>
              </div>
              <div className="col-lg-12">
                <p className="text-muted" style={{ fontSize: 13 }}>Blog article cards are inserted manually from existing blog posts.</p>
              </div>
            </>
          )}
        </SectionCard>
          ...
          {whyChooseShow && (
            <>
              <div className="col-lg-12">
                <div className="my_profile_setting_input form-group">
                  <label>Section Heading</label>
                  <input type="text" className="form-control" value={whyChooseHeading} onChange={e => setWhyChooseHeading(e.target.value)} placeholder="e.g. Why UK Teams Choose Akoode Over Local Agencies" />
                </div>
              </div>
              <div className="col-lg-12">
                <div className="my_profile_setting_textarea form-group">
