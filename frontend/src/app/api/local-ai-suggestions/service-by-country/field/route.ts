...
async function proxySbcAiRequest(req: Request, endpoint: string) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Local AI field suggestion proxy failed";
    return Response.json({ status: "error", message }, { status: 502 });
  }
}

export async function POST(req: Request) {
  return proxySbcAiRequest(req, "/admin/api/ai-suggestions/service-by-country/field");
}
