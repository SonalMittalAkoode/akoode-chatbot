...
const { toSlug, splitCountrySlug } = require("./slugUtils");
const common = require("./commonMapper");

async function mapCountryDocument(sections) {
  const warnings = [];
  const core = sections.core ? sections.core.fields : {};

  const title = pickText(core, ["title"]);
  const keywords = pickText(core, ["keywords"]);

  const rawSlug = pickText(core, ["slug"]);
  const fromPath = splitCountrySlug(rawSlug);
  const slug = fromPath.slug || (title ? toSlug(title) : "");
  const market = pickText(core, ["market"]) || fromPath.market;

  const rawCountry = pickText(core, ["country"]) || pickText(core, ["city"]);
  const country = normalizeCountryLabel(rawCountry) || market;
  if (rawCountry && country !== rawCountry) {
    warnings.push(
      `The document's "Country" field carried extra notes ("${rawCountry.slice(0, 60)}${rawCountry.length > 60 ? "…" : ""}"). Only "${country}" was imported — this value is shown as the breadcrumb on every linked city page.`
    );
  }
...
module.exports = { mapCountryDocument };
